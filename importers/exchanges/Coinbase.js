...
Fetcher.prototype.getTrades = function(sinceTid, callback) {
  ...
  const fetch = cb => this.Coinbase_public.getProductTrades(this.pair, { after: sinceTid, limit: BATCH_SIZE }, this.processResponse('getTrades', cb));
...
Fetcher.prototype.findFirstTrade = function(sinceTs, callback) {
  ...
  const handle = (err, data) => {
    ...
    setTimeout(() => {
      const fetch = cb => this.Coinbase_public.getProductTrades(this.pair, { after: nextScanId, limit: 1 }, this.processResponse('getTrades', cb));
      retry(null, fetch, handle);
    }, QUERY_DELAY);
  }

  const fetch = cb => this.Coinbase_public.getProductTrades(this.pair, { limit: 1 }, this.processResponse('getTrades', cb));
  retry(null, fetch, handle);
}
...
let handleFetch = (err, trades) => {
  if (err) {
    log.error(`There was an error importing from Coinbase ${err}`);
    fetcher.emit('done');
