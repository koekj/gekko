...
const Trader = function(config) {
  ...
  this.Coinbase_public = new Coinbase.PublicClient(
    this.use_sandbox ? this.api_sandbox_url : undefined
  );
  this.Coinbase = new Coinbase.AuthenticatedClient(
    this.key,
    this.secret,
    this.passphrase,
    this.use_sandbox ? this.api_sandbox_url : undefined
...
const recoverableErrors = [
  ...
  'Rate limit exceeded',
  'Response code 5',
  'Coinbase is currently under maintenance.',
  'HTTP 408 Error',
  'HTTP 504 Error',
  'HTTP 503 Error',
  'socket hang up',
...
Trader.prototype.getPortfolio = function(callback) {
  ...
  const fetch = cb =>
    this.Coinbase.getAccounts(this.processResponse('getPortfolio', cb));
...
Trader.prototype.getTicker = function(callback) {
  ...
  const fetch = cb =>
    this.Coinbase_public.getProductTicker(this.pair, this.processResponse('getTicker', cb));
  retry(null, fetch, result);
};

Trader.prototype.getFee = function(callback) {
  //https://www.Coinbase.com/fees
  // const fee = this.asset == 'BTC' ? 0.0025 : 0.003;
  const fee = 0;

  //There is no maker fee, not sure if we need taker fee here
  //If post only is enabled, Coinbase only does maker trades which are free
...
Trader.prototype.buy = function(amount, price, callback) {
  ...
  const fetch = cb =>
    this.Coinbase.buy(buyParams, this.processResponse('buy', cb));
  retry(null, fetch, result);
};
...
Trader.prototype.sell = function(amount, price, callback) {
  ...
  const fetch = cb =>
    this.Coinbase.sell(sellParams, this.processResponse('sell', cb));
  retry(null, fetch, result);
};
...
Trader.prototype.checkOrder = function(order, callback) {
  const result = (err, data) => {
    ...
    // @link:
    // https://stackoverflow.com/questions/48132078/available-Coinbase-order-statuses-and-meanings
  ...
  const fetch = cb =>
    this.Coinbase.getOrder(order, this.processResponse('checkOrder', cb));
  retry(null, fetch, result);
};
...
Trader.prototype.getOrder = function(order, callback) {
  const result = (err, data) => {
    ...
    const fees = {
      // you always pay fee in the base currency on Coinbase
  ...
  const fetch = cb =>
    this.Coinbase.getOrder(order, this.processResponse('getOrder', cb));
  retry(null, fetch, result);
};
...
Trader.prototype.cancelOrder = function(order, callback) {
  ...
  const fetch = cb =>
    this.Coinbase.cancelOrder(order, this.processResponse('cancelOrder', cb));
  retry(null, fetch, result);
};
    ...
    if (this.scanback) {
      ...
      if (!this.scanbackTid) {
        ...
        if (moment.utc(last.time) < moment.utc(since)) {
          ...
          setTimeout(() => {
            let handler = cb =>
              this.Coinbase_public.getProductTrades(
                this.pair,
                {
